...
public class UtilitytoolsApplication implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    ...
    if (args.length ==  1 && args[0].equals("h")){
      ...
      System.out.println("17.5 use 'callFromErrorLines' for each line of tsv file which generated by 17 and there is all instances which faces error");
      System.out.println("\t \t callFromErrorLines [tsv error file] [destination file] [api to call] [api for check status] [is continuing] [file to save progress]");
...
//http://localhost:5000/textsearch?text=
              ...
              while ((line = reader.readLine()) != null) {
                  ...
                  try {
                      ...
                      if(tempresults.length()==0){
                          ...
                          addForCheck(id, claim, label,"no result"+responseAsJson.toString().trim().replace("\n"," "), destinationFilePath);
                      }else{
                      ...
                      //ex.printStackTrace();
                      addForCheck(id, claim, label, ex.getMessage(), destinationFilePath);
                  }
              }
          } catch (IOException e) {
              System.out.println("An error occurred while reading the file: " + e.getMessage());
      ...
      // 17.5 callFromErrorLines of (17) [tsv error file] [destination file] [api to call] [api for check status] [is continuing] [file to save progress]
      if(args.length == 7 && args[0].equals("callFromErrorLines")){
          String filePath = args[1];
          String destinationFilePath = args[2];
          String api = args[3];
          String apistatusCheck = args[4];
...
//http://localhost:5000/check?text=
          APICaller apic = new APICaller(api,destinationFilePath, apistatusCheck,progressfilePath);
          if(isContinuing.equals("no")) {
              HashMap<String, Boolean> tsvs = new HashMap<>();
              try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
                  String line;
                  while ((line = reader.readLine()) != null) {
                      tsvs.put(line, false);
    ...
    private void addForCheck(BigInteger id, String claim, String label,String reason,String destinationFilePath) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(destinationFilePath+".tsv.error", true))) {
            writer.write(id+"\t"+label+"\t"+claim+"\t--"+reason);
            writer.newLine();
        } catch (IOException e) {
            e.printStackTrace();
        }
